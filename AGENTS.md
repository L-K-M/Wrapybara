...
- **A miniaturised window, ⌘H, or a background native tab must not mark its page
  hidden either** (`SiteWindow`). These make `NSWindow.isVisible` false while the
  app is still frontmost — no WebKit switch reaches that check, so site-app windows
  override `isVisible` to report visible until their close begins, then answer
  honestly again for AppKit's quit/reopen logic.
