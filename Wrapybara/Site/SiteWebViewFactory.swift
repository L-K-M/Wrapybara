...
enum SiteWebViewFactory {
    ...
    static func preventHiddenPageThrottling(_ configuration: WKWebViewConfiguration) {
        ...
        // linked against the macOS 14 SDK are also subject to a RunningBoard-based
        // mechanism that suspends or throttles the WebContent process of a page
        // that isn't visible, independently of those keys. `.none` opts out, so a
        // page WebKit does consider hidden (a sleeping display, another Space)
        // keeps running instead of freezing mid-stream. A typed property rather
        // than KVC, so a retirement is a compile error, not an uncatchable throw.
