...
final class UpdateChecker: ObservableObject {
    ...
    /// Brings the alert forward before running it modally — a background check can
    /// complete while Wrapybara isn't the active app, and without this the alert can
    /// land behind other windows. The level clears transient panels (the element
    /// picker's overlay, a Boost editor sheet) so the modal can never end up
    /// *behind* one — an invisible modal that freezes input.
    @MainActor
